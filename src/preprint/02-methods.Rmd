...
first_date = as.Date("2020-12-01")
first_date_vacc = as.Date("2020-12-08")
...
df_uk = data.frame(date = seq(first_date, last_date, 1)) %>% 
  left_join(deaths_UK, by = c("date" = "date_start")) %>% 
  fill(deaths, .direction = "up") %>% 
  fill(iso, .direction = "updown") %>% 
  mutate(deaths_daily = deaths / 7) %>% 
  mutate(deaths_smoothed = rollmean(deaths_daily, k = 7, fill = NA, align = "right"))

df_us = data.frame(date = seq(first_date, last_date, 1)) %>% 
  left_join(deaths_USA, by = c("date" = "date_start")) %>% 
  fill(deaths, .direction = "up") %>% 
  fill(iso, .direction = "updown") %>% 
  mutate(deaths_daily = deaths / 7) %>% 
  mutate(deaths_smoothed = rollmean(deaths_daily, k = 7, fill = NA, align = "right"))

deaths_reported = bind_rows(df_uk, df_us) %>% 
  mutate(iso3c = iso)

data_for_plot = deaths_reported %>% 
  left_join(baseline_vaccination, by = c("iso3c", "date")) %>% 
  left_join(population, by = "iso3c") %>% 
  filter(date <= last_date & date >= first_date_vacc & !is.na(deaths) ) %>% 
  mutate(
