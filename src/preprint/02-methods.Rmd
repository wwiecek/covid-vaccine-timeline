...
The shape parameters $\alpha_e^P, \beta_e^P$ that appear in Table \@ref(tab:trajectory-sampling-parameters) are the shape parameters associated with a beta distribution with mean $\mu_e^P$ and variance $0.005$. The mean $\mu_e^P$ is determined by estimating the efficacy of each vaccine platform $P$ against each variant (see Table \@ref(tab:vaccine-efficacy)). In the US, we assumed that 100% of vaccines administered used the mRNA platform (data indicates that 90% of vaccines administered in the US were of this type [@mathieuCoronavirusPandemicCOVID192020]), while in the United Kingdom we assumed that 50% of first and second doses used of the adenovirus platform (as manufactured by Oxford/AstraZenecca) and 50% used the mRNA platform [@ukdepartmentofhealthandsocialcareOneYearAnniversary2022]. We assumed third doses were 100% mRNA for both countries. The mean vaccine durations are determined according to models of antibody decay, and are dependent on the sampled vaccine efficacies $\mu_e^P$ [@watsonoliverj.COVID19LMICReports2022].
...
sampling_dists <- data.frame(
    'Country' = c('USA', 'UK'), 
    'Platforms' = c(
      '100\\% mRNA', '50\\% adenovirus and 50\\% mRNA (100\\% mRNA for booster doses)'))
